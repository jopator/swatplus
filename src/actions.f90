      ...
      real :: curr_sw = 0.      ! Current soil water               JT
      real :: curr_fc = 0.      ! Current soil field capacity      JT
      real :: curr_def = 0.     ! Current soil deficit            JT
      real :: curr_def_adj = 0. ! Adjusted for irrigation efficiency   JT
      ...
      do iac = 1, d_tbl%acts
        ...
        if (action == "y") then
          ...
          ! Jose 2025 - Irrigation back to field capacity
          case ("irr_to_fc")
            ipl = 1
            j = ob_cur                      ! hru number

            iob = d_tbl%act(iac)%ob_num     ! source object type number
            irrop = d_tbl%act_typ(iac)      ! irrigation application type in irr.ops

            ! Calculate deficit of water required to bring soil water back to FC
            curr_fc = soil(j)%sumfc         ! mm H20
            curr_sw = soil(j)%sw            ! mm H20
            curr_def = curr_fc - curr_sw    ! mm H20

            if (curr_def <= 0.0) then
              curr_def = 0.0
              curr_def_adj = 0.0
            else
              curr_def_adj = curr_def / ((irrop_db(irrop)%eff)*(1. - irrop_db(irrop)%surq)) !Increase actual amount to consider irrigation efficiency
            end if

            irrig(j)%applied = curr_def_adj
            irrig(j)%runoff  = curr_def_adj * irrop_db(irrop)%surq
            irrig(j)%demand  = curr_def_adj * hru(j)%area_ha * 10.  ! m3 actual demand is based on deficit + considering extras through irrigation efficiency and surface runoff
            
            ! add irrigation to yearly sum for dtbl conditioning jga 6-25
            hru(j)%irr_yr = hru(j)%irr_yr + irrig(j)%applied

            if (pco%mgtout == "y") then
              write (2612, *) j, time%yrc, time%mo, time%day_mo, d_tbl%act(iac)%name, "IRR_TO_FC", phubase(j),  &
          ...
          case ("res_irr_dmd")
            ...
            res_ob(iob)%d_irrig_day = d_tbl%act(iac)%const * irrig(j)%demand  !const is equal to the % of the demand that corresponds to that res
